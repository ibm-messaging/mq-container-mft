...
4. Download **9.3.5.0-IBM-MQFA-Redist-LinuxX64.tar.gz** or higher from [IBM Fixcentral](https://www.ibm.com/support/fixcentral/) into the current directory.
5. Unpack the **9.3.5.0-IBM-MQFA-Redist-LinuxX64.tar.gz** to a temporay directory. Copy com.ibm.wmqfte.com.ibm.wmqfte.exitroutines.api.jar to credentialsexit/BridgeCredentialExit/thirdparty directory.
6. Download json-20240205.jar file from [Maven Repository](https://mvnrepository.com/artifact/org.json/json/20240205) and copy to credentialsexit/BridgeCredentialExit/thirdparty directory.
...
7. Run the following command to build container image
   ...
   `podman build -f Dockerfile-agent -t mqmft:9.3.5.0 --build-arg ARG_MQMFT_REDIST_FILE=9.3.5.0-IBM-MQFA-Redist-LinuxX64.tar.gz .`
   
   You can replace the `9.3.5.0-IBM-MQFA-Redist-LinuxX64.tar.gz` with the version of the redistributable package of your choice.
